...
const getAllUsers=async(req,res)=>{
    const users = await User.find({ role: { $in: ["tenant", "owner"] } }).select('-password')
...
const updateUser=async(req,res)=>{
    ...
    if(!user){
        throw new CustomError.NotFoundError('The user with the given email does not exist')
    }
...
const updatePassword=async(req,res)=>{
    ...
    if(!oldPassword || !newPassword){
        throw new CustomError.BadRequestError('Please provide oldPassword as well as newPassword')
    }
    ...
    await user.save()
    res.status(StatusCodes.OK).json({msg:'Succcess !! Password updated successfully'})
}
...
const bookPG=async(req,res)=>{
    ...
    if(!pg){
        throw new CustomError.NotFoundError('The PG with the given id is not found')
    }
    ...
    if(user.role!=='tenant'){
        throw new CustomError.BadRequestError('You are not allowed to book a PG since you are not tenant')
    }
...
const deleteUser=async(req,res)=>{
    ...
    const user=await User.findOne({_id:userId})
    if(!user){
        throw new CustomError.BadRequestError('The id of the given user does not exists')
    }
    ...
    await user.remove()
    res.status(StatusCodes.OK).json({msg:`user with id:${userId} deleted successfully`,user})
}
